...
**✅ RESOLVED (2026-06-21, option A):** rebuilt the exam bank on the v3 blob pattern. New endpoints `POST /api/acad/exam/{create,list,delete,assign}` authenticate via the **normal session token** (`_resolve_token`) and store per-lecturer via `db.coll_put("acad_exams", id, …, owner=sid)` (+ `acad_class_exams` for assignments, `owner=code`) — mirrors `acad_assignment_*`. Owner-scoped (a lecturer sees/deletes only their own) and **id-based delete** (kills the index race). Assign requires `_acad_require_owner(code, sid)` on a v3 class + exam ownership; members get a push. Frontend `lLoadExams/lCreateExam/lDeleteExam/lAssignExam` rewired to the new endpoints (delete now passes `e.id`). Old `/api/lecturer/exam*` (global `LECTURER_PASSWORD` bank) left intact for the legacy `/lecturer` dashboard. Storage logic verified against `coll_*` semantics; syntax-clean. **NOT browser-verified.** Student take-flow for v3 exams still deferred (the assignment is persisted in `acad_class_exams` for when it's built).
