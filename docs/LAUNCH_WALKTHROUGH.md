...
| **Logo asset weight** | 🚩 | `static/sivarrai.png` is **256 KB**, rendered at 16–24 px in-app (also `apple-touch-icon` ~180 px + `og:image` for social previews — so it can't be globally shrunk to one tiny size). **Fix (needs an image tool — Pillow/ImageMagick, not in this env):** ship a small `sivarrai-32.png` (or SVG) for the 9 in-app `<img>` refs, keep a ~180 px copy for `apple-touch-icon`, and a ~1200×630 copy for `og:image`. Saves ~250 KB on every cold load. |
...
| **TipTap from esm.sh** | 🚩 | Docs/Notes editor imports `@tiptap/*` from `https://esm.sh` at module load. External CDN = single point of failure for the Docs editor (same risk class as the old jsdelivr issue). **Fix (needs network — blocked in the offline dev env):** on a networked machine, fetch each bundled module — `curl -L "https://esm.sh/@tiptap/core@2.10.3?bundle&target=es2020" -o static/vendor/tiptap/core.js` (repeat for `starter-kit`, `extension-placeholder`, `extension-underline`), then point the `<head>` `import` lines at `/static/vendor/tiptap/*.js`. Verify the Docs editor still mounts before deploy. |
| Index served uncached | ✅ | `_serve_app()` now builds the templated HTML **once and caches it in memory** (`_APP_HTML_CACHE`); injected config is all static env-derived values. Dev (`RAILWAY_ENVIRONMENT=development`) still re-reads each request for template hot-reload. No more 288 KB disk read + `.replace()` per `/app` hit. |
