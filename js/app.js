...
function dhLoadDocs() {
  try {
    return JSON.parse(localStorage.getItem(DH_KEY()) || "[]");
  } catch {
    return [];
  }
}
function dhSaveDocs(docs) {
  localStorage.setItem(DH_KEY(), JSON.stringify(docs));
}
...
function dhTriggerUpload() {
  $("dh-upload-input").click();
}
...
function dhHandleUpload(input) {
  ...
  files.forEach((file) => {
    if (file.size > 8 * 1024 * 1024) {
      toast(`"${file.name}" exceeds 8 MB limit — skipped.`);
      finish();
      return;
    }
    ...
    reader.onload = () => {
      const doc = {
        ...
        title: file.name,
        content: isText ? reader.result : "",
        updated: Date.now(),
        kind: isText ? "doc" : "file",
        fileType: file.type || "application/octet-stream",
        fileName: file.name,
        fileSize: file.size,
        fileData: isText ? null : reader.result,
      };
...
function _dhFileLabel(mimeType, fileName) {
  ...
  if (mimeType?.includes("officedocument") || /\.docx$/i.test(fileName))
    return "DOCX";
...
function _dhImageLightbox(doc) {
  ...
  ov.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.87);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:1rem";
  ov.onclick = () => ov.remove();
  const img = document.createElement("img");
  img.src = doc.fileData;
  img.style.cssText =
    "max-width:100%;max-height:100%;border-radius:8px;box-shadow:0 20px 60px rgba(0,0,0,.5)";
  ov.appendChild(img);
...
async function dhAISuggest() {
  ...
  if (!content || content.length < 20) {
    toast("Write some content first for AI to suggest.");
    return;
  }
  ...
  try {
    ...
    suggestion.style.cssText =
      "border-left:3px solid var(--accent2);padding:.5rem .75rem;background:#7c3aed08;border-radius:0 6px 6px 0;margin:.5rem 0;font-size:.85rem;color:var(--muted)";
    suggestion.innerHTML = `<strong style="color:var(--accent2)">✨ AI Suggestion:</strong><br>${r.reply}`;
    $("dh-editor").appendChild(suggestion);
    dhAutoSave();
  } catch (e) {
    toast("AI suggestion failed — try again.");
  }
}

async function dhDeleteDoc(id) {
  if (
    !(await siModal.confirm("This document will be permanently deleted.", {
...
function sgShowCreate() {
  $("sg-create-form").style.display =
    $("sg-create-form").style.display === "none" ? "block" : "none";
  $("sg-join-form").style.display = "none";
  $("sg-name-input")?.focus();
}
function sgShowJoin() {
  $("sg-join-form").style.display =
    $("sg-join-form").style.display === "none" ? "block" : "none";
...
async function sgJoin() {
  ...
  try {
    ...
    toast("Invalid group ID or already a member.");
  }
}
...
function sgOpenChat(gid, gname) {
  ...
  const av = $("sg-chat-av");
  if (av) av.textContent = gname[0].toUpperCase();
  const nm = $("sg-chat-name");
  if (nm) nm.textContent = gname;
  const id = $("sg-chat-id");
  if (id) id.textContent = `ID: ${gid}`;
  const box = $("sg-messages");
...
function sgCopyId() {
  if (!SG_ACTIVE) return;
  navigator.clipboard
    .writeText(SG_ACTIVE.id)
    .then(() => toast("Group ID copied ✓"));
}
...
function sgConnectStream() {
  ...
  try {
    ...
    SG_SSE = new EventSource(
      `/api/group/chat/stream?group_id=${encodeURIComponent(SG_ACTIVE.id)}&last_id=${SG_LAST_ID}`,
    );
    SG_SSE.onmessage = (e) => {
      try {
        sgAppendMsg(JSON.parse(e.data));
      } catch (_) {}
    };
    SG_SSE.onerror = () => {
      ...
      if (!SG_INTERVAL && SG_ACTIVE)
        SG_INTERVAL = setInterval(() => sgLoadMessages(false), 4000);
    };
  } catch (_) {
    if (!SG_INTERVAL)
      SG_INTERVAL = setInterval(() => sgLoadMessages(false), 4000);
...
function sgAppendMsg(m) {
  ...
  const empty = box.querySelector(".sg-empty");
...
function renderNotes(notes, filter = "") {
  ...
  if (filter === "__search__") {
    ...
    display = notes.filter(
      (n) => (n.tag || "").toLowerCase() === filter.toLowerCase(),
    );
  }
  if (!display.length) {
  ...
  list.innerHTML = display
    .map((n, i) => {
      ...
      const title =
        lines[0].slice(0, 55) +
        (lines[0].length > 55 || lines.length > 1 ? "..." : "");
...
function noteRenderTagBar(notes) {
  ...
  bar.innerHTML = [
    `<button onclick="noteFilterTag('',this)" style="padding:4px 12px;border-radius:20px;border:1px solid var(--border);background:var(--card);color:var(--muted);font-family:var(--font);font-size:.72rem;font-weight:700;cursor:pointer;white-space:nowrap;transition:all .15s" class="note-tag-filter active">All</button>`,
    ...tags.map(
      (t) =>
        `<button onclick="noteFilterTag('${esc(t)}',this)" style="padding:4px 12px;border-radius:20px;border:1px solid var(--border);background:var(--card);color:var(--muted);font-family:var(--font);font-size:.72rem;font-weight:700;cursor:pointer;white-space:nowrap;transition:all .15s" class="note-tag-filter">${esc(t)}</button>`,
    ),
  ].join("");
}
...
function saveNote() {
  ...
  const editIdx =
    ta.dataset.editIdx !== undefined && ta.dataset.editIdx !== ""
      ? parseInt(ta.dataset.editIdx)
      : -1;
